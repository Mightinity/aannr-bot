...
function getCPUPercentage() {
    return new Promise((resolve, reject) => {
        const pyshell = new PythonShell('./pyfunction/cpupercent.py')
        pyshell.on('message', (result) => {
            const cpuPercent = parseFloat(result);
            resolve(cpuPercent);
        })
        pyshell.end((err) => {
            if (err){
...
module.exports = { getCPUPercentage, getUpTime, getMemoryInfo, getCPUInfo}